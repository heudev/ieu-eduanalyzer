...
const initialState: CourseState = {
    ...
    faculties: departmentsData as Faculty[],
};
...
export const courseSlice = createSlice({
    ...
    reducers: {
        ...
        addDepartment: (state, action: PayloadAction<Department>) => {
            if (!state.selectedDepartments.find((dept: Department) => dept.id === action.payload.id)) {
        ...
        removeDepartment: (state, action: PayloadAction<string>) => {
            state.selectedDepartments = state.selectedDepartments.filter(
                (dept: Department) => dept.id !== action.payload
            );
        ...
        loadInitialData: (state) => {
            state.faculties = departmentsData as Faculty[];
            saveState(state);
        },
    },
});
