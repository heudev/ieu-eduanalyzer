...
// Her derse benzersiz id atayan yardımcı fonksiyon
const assignIdsToData = (data: Faculty[]): Faculty[] => {
    return data.map(faculty => ({
        ...
        departments: faculty.departments.map(department => ({
            ...
            courses: department.courses.map(course => ({
                ...course,
                id: uuidv4(),
                letterGrade: course.letterGrade || 'NA',
...
const initialState: CourseState = {
    ...
    faculties: assignIdsToData(departmentsData),
};
...
export const courseSlice = createSlice({
    ...
    reducers: {
        ...
        loadInitialData: (state) => {
            state.faculties = assignIdsToData(departmentsData);
            saveState(state);
        },
    },
});
